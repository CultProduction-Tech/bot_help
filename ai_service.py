...
Ты должен извлечь:
1. Компанию, для которой нужно создать проект.
   НЕ изменяй и НЕ сокращай название проекта — имя берётся из текста пользователя как есть.
2. Если пользователь указал только ID сделки AmoCRM (число из 5+ цифр), company можно определить по контексту.
